...
export class CourseService {
  ...
  async createCourseFromRoadmap(params: CourseCreationParams): Promise<ICourse> {
    ...
    if (roadmap.courseId) {
      const existingCourse = await this.getCourseById(roadmap.courseId, userId);
      if (existingCourse) {
    ...
    const detailedModules = await this.generateDetailedModules(roadmap.title, roadmap.modules);
    ...
    await linkRoadmapToCourse(roadmapId, (course._id as string).toString());
    
    return course;
  }
  
  async getCourseById(courseId: string, userId: string): Promise<ICourse | null> {
  ...
  private async generateDetailedModules(courseTitle: string, roadmapModules: any[]): Promise<IModule[]> {
    console.log(`Generating detailed content for course: ${courseTitle}`);
    
    const detailedModules: IModule[] = [];
    
    // Process each module one by one to avoid overloading the AI with too much text
    for (const roadmapModule of roadmapModules) {
      const detailedLessons = await this.generateDetailedLessonsForModule(courseTitle, roadmapModule);
  ...
  private async generateCourseDescription(title: string, complexity: number, modules: any[]): Promise<string> {
    try {
      ...
      const topicAreas = modules.slice(0, 3).map(module => 
        module.title
